...
export const seedDatabase = () => {
    return new Promise((resolve, reject) => {
        ...
        db.serialize(() => {
            ...
            const markets = [
                { id: 1, name: "ตลาดนัดจตุจักร", description: "ตลาดนัดสุดสัปดาห์ที่ใหญ่ที่สุด", owner_id: 30, location: "จตุจักร, กรุงเทพฯ", open_time: "09:00", close_time: "18:00", rating: 4.8 },
                { id: 2, name: "ตลาดนัดรถไฟรัชดา", description: "ตลาดกลางคืนชื่อดัง", owner_id: 30, location: "รัชดาภิเษก, กรุงเทพฯ", open_time: "17:00", close_time: "01:00", rating: 4.5 },
                { id: 3, name: "ตลาดนัดหลังมอ", description: "ตลาดนัดราคาถูก", owner_id: 31, location: "รามคำแหง, กรุงเทพฯ", open_time: "16:00", close_time: "22:00", rating: 4.2 },
                { id: 4, name: "ตลาดนัดใหม่เอี่ยม", description: "ตลาดเปิดใหม่ รอการอนุมัติ", owner_id: 32, location: "ลาดพร้าว, กรุงเทพฯ", open_time: "10:00", close_time: "20:00", rating: 0 },
            ];

            const marketSql = `INSERT OR REPLACE INTO markets (id, name, description, owner_id, location, open_time, close_time, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
            markets.forEach((m) => {
                db.run(marketSql, [m.id, m.name, m.description, m.owner_id, m.location, m.open_time, m.close_time, m.rating]);
            });
            ...
            const favorites = [
                { user_id: 10, seller_id: 20 }, { user_id: 10, seller_id: 21 }, { user_id: 10, seller_id: 24 },
                { user_id: 11, seller_id: 22 }, { user_id: 11, seller_id: 24 },
                { user_id: 12, seller_id: 20 }, { user_id: 12, seller_id: 23 }, { user_id: 12, seller_id: 24 },
            ];
            ...
            favorites.forEach((f, index) => {
                db.run(favoriteSql, [f.user_id, f.seller_id], (err) => {
                    // Resolve หลัง insert ตัวสุดท้าย
